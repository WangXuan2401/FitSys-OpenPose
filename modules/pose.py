...
from modules.calculation_score import *
...
class Pose:
    ...
    def __init__(self, keypoints, confidence, standards):
        super().__init__()
    ...
    def draw(self, img):
        """
        Draw the skeletons of the input image frame.
        """
        assert self.keypoints.shape == (Pose.num_kpts, 2) # (18, 2)
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1:
                ...
                cv2.circle(img, (int(x_a), int(y_a)), 4, [0, 0, 255], -1)
                cv2.putText(img, str(kpt_a_id), (int(x_a), int(y_a)-16), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            
            #  tips of limbs and head key points
            kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
            global_kpt_b_id = self.keypoints[kpt_b_id, 0]
            if global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.circle(img, (int(x_b), int(y_b)), 4, [255, 0, 0], -1)
                cv2.putText(img, str(kpt_b_id), (int(x_b), int(y_b)-16), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
            
            # connecting line            
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                cv2.line(img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), Pose.color, 2)

    def get_show_score(self, img):
        """
        Calculate the pose score of input image and show the score in the image.
        """
        assert self.keypoints.shape == (Pose.num_kpts, 2)
        scores = cal_score(self.keypoints, self.standards)
        draw_scores(scores, img, self.standards, self.bbox)
        return scores
